// ============================================================
//  main.js — ส่วน JavaScript ของเว็บจองที่พัก
//  หน้าที่: โหลดข้อมูลโรงแรม → สร้างการ์ด → กรองตามเมือง
// ============================================================

// เก็บโรงแรมทั้งหมดที่โหลดมา ไว้ใช้ตอนกรอง
let allHotels = [];

// ------------------------------------------------------------
// 1) โหลดข้อมูลโรงแรมจาก hotels.json
//    fetch() = ขอข้อมูลจาก "หลังบ้าน" (ภาพของ backend)
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// 2) สร้างการ์ดโรงแรมลงในหน้าเว็บ
// ------------------------------------------------------------
function renderHotels(list) {
  const grid = document.getElementById('hotel-grid');
  grid.innerHTML = list.map(cardHTML).join('');

  // 👇 โจทย์ 6: ตัวเลข "พบ ... ที่พัก" ค้างที่ 0 เสมอ — อัปเดตให้ตรงกับจำนวนที่แสดงอยู่ (list.length)
  //    ใบ้: document.getElementById('result-count').textContent = `พบ ${list.length} ที่พัก`;
}
...
// ------------------------------------------------------------
// 3) กรองโรงแรมตามเมือง
//    👇 โจทย์ 5: ตอนนี้กดปุ่มกรองแล้วยังไม่มีอะไรเกิดขึ้น
//       ทำให้มันทำงาน โดย:
//         - ถ้าเลือก "ทั้งหมด" → แสดงทุกโรงแรม
//         - ถ้าเลือกเมืองอื่น → แสดงเฉพาะโรงแรมในเมืองนั้น
//       (ตัวช่วย: ใช้ allHotels.filter(...) แล้วส่งให้ renderHotels)
// ------------------------------------------------------------
function filterHotels(city) {
  // TODO: เขียนโค้ดตรงนี้ (ลองให้ AI ช่วย)
}

// ------------------------------------------------------------
//  โค้ดช่วยเหลือ (ทำงานได้แล้ว ไม่ต้องแก้)
...
// เริ่มทำงาน: โหลดโรงแรมเมื่อเปิดหน้าเว็บ
loadHotels();
